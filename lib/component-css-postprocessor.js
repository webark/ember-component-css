...
function ComponentCssPostprocessor(inputTree, options) {
  ...
  var checker = new VersionChecker(options.addon);
  var dep = checker.for('ember', 'bower');

  var scriptLocation;
  if (dep.lt('2.5.0-beta.1')) {
    scriptLocation = path.join(options.addon.root, 'lib/lookup-reopen/legacy.js');
  } else {
    scriptLocation = path.join(options.addon.root, 'lib/lookup-reopen/new.js');
  }
  this.reopenScript = fs.readFileSync(scriptLocation).toString();
}
