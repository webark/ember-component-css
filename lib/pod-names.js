...
PodNames.prototype.build = function() {
  ...
  this._patches = this._patches.concat(patches);

  return Promise.resolve().then(this.writePodStyleName.bind(this, this._patches));
};
...
PodNames.prototype.writePodStyleName = function(patches) {
  ...
  console.log('fs.writeFileSync', path.join(this.outputPath, 'pod-names.js'));
