...
PodNames.prototype.addClass = function(stylePath) {
  var componentPath = componentNames.path(stylePath, this.classicStyleDir),
    componentClass = `${this.prefix}__${componentNames.class(stylePath, this.classicStyleDir, this.terseClassNames)}`;
  if (this.prefix !== '') {
    componentPath = componentPath.replace('components/', `components/${this.prefix}@`)
  }
