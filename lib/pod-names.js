...
PodNames.prototype.writePodStyleName = function(patches) {
  ...
  var currentPodNames = {};
  var outputPathDir = path.dirname(this.outputPath);

  var podDirectories = Finder.from(outputPathDir).findDirectories('pod_names-output_path-<[a-zA-Z0-9]+>.tmp');

  podDirectories.push(this.outputPath);

  podDirectories.forEach((podDirectory) => {
    let filePath = path.join(podDirectory, 'pod-names.js');

    if (fs.existsSync(filePath)) {
      let contents = fs.readFileSync(filePath).toString();
      let podNames = JSON.parse(contents.substr(contents.indexOf('{')));
      currentPodNames = Object.assign(currentPodNames, podNames);
    }
  });
